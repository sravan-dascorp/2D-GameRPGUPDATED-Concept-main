...
namespace RPGM.Gameplay
{
    ...
    public class Quest : MonoBehaviour
    {
        ...
        public bool externalvariable_change;
        public Booltrigger booltrigger;
        ...
        public bool IsQuestComplete()
        {
            ...
            {
                if (inv.Contains(i.item.name) && model.GetInventoryCount(i.item.name) >= i.count || questionAnswered) {  if(externalvariable_change) booltrigger.execute=true; popup.SetActive(false);  continue; }
                return false;
            }
            return true;
        }
        ...
        {
            questionclick.spacebuuttoncliced = true;
            
            Vector2 currentposition = player.transform.position;
            player.transform.position = currentposition + new Vector2(0, -0.4f);
