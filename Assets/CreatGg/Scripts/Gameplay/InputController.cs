...
namespace RPGM.UI
{
    ...
    public class InputController : MonoBehaviour
    {
        ...
        void DialogControl()
        {
           // model.player.nextMoveCommand = Vector3.zero;
            if (Input.GetKeyDown(KeyCode.LeftArrow))
                ...
                model.dialog.FocusButton(+1);
            if (questionclick.spacebuuttoncliced)
            {
                 model.dialog.SelectActiveButton();
                 questionclick.spacebuuttoncliced=false;
            }
        ...
        {
            Vector2 movement = new Vector2(SimpleInput.GetAxis("Horizontal"), SimpleInput.GetAxis("Vertical"));
            movement.Normalize();
            model.player.nextMoveCommand = movement * stepSize;
